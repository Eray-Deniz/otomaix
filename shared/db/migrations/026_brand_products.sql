...
BEGIN
    ...
    LOOP
        ...
        IF mevcut_def IS NOT NULL AND mevcut_def <> 'plpgsql|trigger|kanonik' THEN
            RAISE EXCEPTION
                'migration 026: %() adini KANONIK OLMAYAN bir fonksiyon tutuyor (%)',
                kayit.ad, mevcut_def
    ...
    FOR kayit IN
        SELECT * FROM (VALUES
            ('social.brand_products', 'brand_products_updated_at',
             'CREATE TRIGGER brand_products_updated_at BEFORE UPDATE ON social.brand_products FOR EACH ROW EXECUTE FUNCTION social.set_updated_at()')
        ) AS t(tablo, ad, tanim)
    LOOP
        ...
        IF mevcut_def IS NOT NULL AND mevcut_def <> kayit.tanim || '|O' THEN
            RAISE EXCEPTION
                'migration 026: % adini KANONIK OLMAYAN bir tetikleyici tutuyor (%)',
                kayit.ad, mevcut_def
                USING ERRCODE = 'integrity_constraint_violation',
