...
| **R-D3** | Motor Görev B bağını `CIddia.anahtarlar` KÜMESİNDEN kurar; `normalize_special_day_key` bu yolda ÇAĞRILMAZ; `_iddia_alani_bagli_mi(karar_alani, oge_yolu, iddia)` imzası | kapı DAVRANIŞI değişti | EVET — küme boşsa `donem-kimligi-cozulemedi` (dürüst teşhis KORUNDU), başka dönemin anahtarı `iddia-alani-uyusmuyor` (2026-09-20'ye kadar `iddia-arastirmada-yok`'tu; iddia araştırmada VARDIR, örtüşmeyen `alan/dönem` hücresidir — motor `2.16.0`) |
