...
> Kanonik kaynak: `/root/otomaix-sosyal-medya-arastirmasi/sektor-bilgi-paketi-spec-input.md` — commit `dc3a427` (2026-08-21), 3136 satır.
> Kaynak sha256: `7f41edec28a2f390894e96ade02073dbc12df510b72aeffa36350242829eb0be`
> Doğrulama: `tail -n +7 <bu dosya> | sha256sum` kaynak sha'sını verir — ilk 6 satır (bu blok + boş satır) hariç bayt-özdeş kopyadır.
> Kaynak depoda Bölüm 17 final sweep'i koşulursa bu kopya yeniden eşitlenir; spec kalıcı atıfları bu kopyanın satırlarına değil karar ID'lerine (K-xx) yapar.
...
- **Önerilen çalışma biçimi:** Turu **Otomaix yöneticisi** koşar; **Otomaix müşterisi bu sürecin katılımcısı değildir** — yalnız kendi markasının alt sektörünü teyit eder (Bölüm 9, 15). Yönetici bir alt sektör için üç bağımsız araştırma çıktısını yükler → iki **kör ve bağımsız** hakem çıktıları iddia düzeyinde denetler → sentez, denetimleri **yürürlükteki paket varsa** onunla karşılaştırıp sürümlü bir `draft` üretir. ⚠️ **İlk pakette aktif paket yoktur**; `koru` / `guncelle` / `cikar` uygulanmaz, bütün kararlar `ekle` evrenindedir (Bölüm 7.5).
  ...
  ⚠️ **Bundan sonrası tek bir akış değildir — K-22'ye bağlı iki modeldir ve bu belge ikisini de taşır, birini seçmez** (Bölüm 5.2, 7.7):
  - **Motor Faz 1'e girerse:** `draft` içeriğini **politika motoru** üretir (sentez çıktısı motorun girdisidir); yönetici koşu sonucunu, değişim oranlarını ve özet diff'i görür.
  - **Girmezse:** aday paketi **sentez** üretir ve `draft` doğrudan sentez çıktısından yazılır; motor koşu raporu üretilmez, yönetici **sentezin özetini, karar günlüğünü, açık soruları ve kapı sonuçlarını** görür.
...
**Karar durumu — Bölüm 17'nin 2026-08-19 ölçümü:**

| Karar durumu | Sayı |
|---|---:|
| Kayıtlı karar | **162** |
| `[AÇIK]` | **157** |
| Kapanmış (**K-03 · K-04 · K-18 · K-145 · K-28**) | **5** |
| **`Spec öncesi kullanıcı kararı`** — spec yazılmadan kapanmalı | **47** |
| Yalnız belirli bir spec / faz / kapsam seçilirse gerekli | 47 |
| Spec içinde teknik olarak çözülür | 35 |
| Bu spec'i bloklamaz | 32 |
| Blok durumu ölçülmedi | 1 |

⚠️ **Kırk yedi karar amaç, kapsam, maliyet, operasyon yükü, politika ve risk kabulü tartar; bunlar spec yazarının çözebileceği kalemler değildir.** Sahiplik tek elde değildir: **42'sinde ürün sahibi** (kimisinde teknik sahiple paylaşımlı), **5'inde yönetici / operatör** (**K-05 · K-10 · K-16 · K-43 · K-44**). Kayıtlı kararların evi **Bölüm 17**'dir; kapanmış kararların günlüğü **Ek B**'de tamamlanacaktır; spec yazarına devir notu **Ek C**'dedir.
...
- **Ele alınma yönü:** Kalıp-başına karar kontrolünün bir **politika motoruna** devredilmesi, insan onayının yalnız aktivasyonda kalması (Bölüm 4.5, 7.7, 15). İki ayrı karar kümesi vardır ve karıştırılmamalıdır:
  - **Motorun fazı** (Faz 1'e mi girer, pilot sonrasına mı) → **K-22**.
...
**Sınır durumu — marka kanal envanteri.** Verinin doğal evi Marka DNA'sıdır ve tam çözümü kapsam dışındadır (3.2). Bu işte kapsam içinde kalan, `[kanal-bağımlı: X]` kalıpların güvenli çalışması için **kullanım-talimatı düzeyinde hafif önlem** alınmasıdır; deterministik filtre, DNA tarafındaki `channels` kararı verildiğinde buraya bağlanır (**K-05**).
...
1. **Motorun fazı — K-22 `[AÇIK]`.** Motorun Faz 1'e mi (pilotla birlikte) yoksa pilot sonrasına mı gireceği karara bağlanmamıştır. Bu karar, elle inceleme yükünün ölçeğine ilişkin **ölçülmemiş** bir gerekçeye dayanır (Bölüm 2.1; **K-21**, **K-13**). Faz kararı, aşağıdaki kalıp kimliği sorusunun kapsam etkisinden **önce** değerlendirilir.
2. **Kalıp kimliği sözleşmesi `[AÇIK]`.** Sürümler arası kalıp kimliğinin (kalıcı kimlik alanı, üretim yöntemi, sürümler arası koruma kuralları) nasıl kurulacağı açıktır; bir hakem kalıcı kimliği zorunlu sayar, diğeri kalıp düzeyinde kalıcı kimlik öngörmez ve sürekliliği karar günlüğü üzerinden metinsel eşleşmeyle kurar. Motor Faz 1'e alınırsa bu sözleşme motor spec'inin ön koşuludur; pilot sonrasına bırakılırsa pilot spec'ini kendiliğinden bloklamaz. Karar ID'si Bölüm 17 sweep'inde verilecektir.
3. **Yönetici koşu yüzeyi — K-27 `[AÇIK]`.** Koşunun Claude Code komut ailesi üzerinden mi, Otomaix paneli üzerinden mi işletileceği açıktır. Bir hakem komut ailesini varsayar, diğeri bunu açık karar olarak kaydeder. Yüzey seçimi işletim prosedürünü (Bölüm 14) ve rol/yetki tasarımını (Bölüm 15) belirler.
...
- **Marka kanal envanterinin tam çözümü (K-05)** — evi belirlenmiştir: Marka DNA dokümanı G1, 5. alan adayı (`channels`). *Koşul:* DNA spec'inde `channels` kararı verildiğinde deterministik filtre bu işe bağlanır. Bu işte yalnız kullanım-talimatı düzeyinde hafif önlem kalır (3.1 sınır durumu).
...
| **Koşu ve aktivasyon rolü** (Otomaix yöneticisi / operatör) `[SEA-2026-08-11]` | Turu koşmak: sektör seç, KAYNAK-1/2/3 yükle, brief'i doldur, koşuyu tetikle, **özet diff'e bakıp aktive et** | Yönetici koşu yüzeyi (**K-27** açık) + özet diff ekranı | Kalıp-kalıp karar **vermez**; koşu sonucuna **yalnız aktivasyon/ret ve rollback onayı** verir; politika motorunu atlayarak aktive edemez. Tur periyodu 3 **veya** 6 ay — **K-26** |
...
> **Statü uyarısı `[SEA-2026-08-11]`:** politika motoru, `no_change` sonucu ve yöneticinin yalnız koşu sonucuna verdiği son onay **kaynak dokümanlardan gelmez**; sonraki ölçeklenebilirlik analizinden gelir. Motorun bu mimariye hangi fazda gireceği **K-22**'de açıktır (Bölüm 3.1, 4.5).
...
> **`draft` içeriğini kim üretir — K-22'ye bağlıdır.** Motor Faz 1'e girmezse aday paketi **sentez** üretir ve `draft` doğrudan sentez çıktısından yazılır; motor Faz 1'e girerse `draft` içeriğini **politika motoru** üretir (sentezin çıktısı o modelde motorun girdisidir). **İki modelde de ortak olan iki hüküm:** `content` doğrulayıcısı **veri tabanına yazımdan önce** çalışır ve reddederse yazım olmaz; kayda **yalnız `draft`** yazılır. Bu bölüm iki modeli de taşır, birini seçmez.
...
**Kararı veren aktör.** Denetçiler **karar vermez, önerir**; sentez karar merciidir; **aktivasyon operatöre** aittir. Motorun hatta bulunduğu modelde kararların uygulanması motora geçer, ancak motorun `active`'e dokunma yetkisi **yoktur** (**K-28**) ve fazı **K-22**'de açıktır.
...
⚠️ **Politika motoru bu omurganın parçası değildir.** Sentez ile onay arasına yerleşen ayrı bir katmandır; hatta hangi fazda gireceği **K-22**'de açıktır. Motorsuz modelde aday paketi sentez üretir ve akış doğrudan biçim doğrulayıcısına geçer (Bölüm 5.2).
...
⚠️ **Klasör ve teslim sözleşmesi açık karardır — K-17.** Yukarıdaki yapı **öneri** statüsündedir; klasör adının `run_id` ile aynı olması da öneri düzeyindedir (Bölüm 6.2). ⚠️ **Klasörün dosya kümesi kapalı değildir:** motorlu modelde aynı klasöre koşunun **politika raporu** ve motorun **nihai adayı** da eklenir (Bölüm 14). Bunların klasörde yer alması motorun hatta girmesine (**K-22**) bağlıdır; ancak **kesin dosya ve veri tabanı yerleşimleri bundan ayrı ve kendi başına açık bir karardır.** Motor Faz 1'e girse bile yerleşim ayrıca kararlaştırılmalıdır. **Yerleşim açıklığı tek değil, ikidir; birleştirilemezler:**
...
- Kümenin gerekçesi **motorun fazına bağlıdır** (**K-22**): mutabakat kapısı motor katmanının kuralıdır, motor Faz 1'e girmezse o fazda kapı da yoktur. Ek görev ise her turda iki denetçiye yük ekler.
...
⚠️ **Bu alt başlığın tamamı hedef modeldir.** Motor, kalıp-başına insan kararının ölçeklenmeyeceği analizinden doğan bir katmandır ve sentez ile aktivasyon arasına yerleşir. Hatta hangi fazda yer alacağı **K-22**'de açıktır (Bölüm 4.5, Bölüm 7.1). **Motorsuz modelde bu alt başlığın hükümleri uygulanmaz:** aday paketi sentez üretir, akış doğrudan biçim doğrulayıcısına ve yönetici onayına geçer.
...
**Yöneticinin gördüğü yüzey motorlu modelde kalıp listesi değildir** — Bölüm 4.5'in otomasyon sınırı orada uygulanır. ⚠️ **Motorsuz modelde aynı şey söylenemez:** yürürlükteki sentez sözleşmesi çıkarılanların **tam listesini** onay ekranının ilk bölümü yapar ve yönetici sentezin karar günlüğüne bakar (Bölüm 7.5). *"Kalıp listesi görülmez"* hükmü bu yüzden **K-22'ye bağlıdır, koşulsuz değildir.**

⚠️ **Aşağıdaki tablo motorlu modelin yüzeyidir.** Motor Faz 1'e girmezse (**K-22**) koşu sonucu, değişim oranları, motorun kararsız kaldıkları ve motor koşu raporu **üretilmez**; motorsuz modelde yönetici **sentezin özetini, karar günlüğünü, açık soruları ve kapı sonuçlarını** görür ve onay bu kümeye dayanır (Bölüm 7.5).
...
⚠️ **Motorlu ↔ motorsuz ikiliği bu bölümde de geçerlidir (K-22).** Aşağıdaki geçişlerin ve ön koşulların bir kısmı politika motorunun varlığını varsayar; motor Faz 1'e girmezse o satırlar **üretilmez**. Motora bağlı olanlar ayrı ayrı işaretlenmiştir.
...
**Marka-gerçeği filtresinin çalışma zamanı ayağı K-05'e bağlıdır.** Kalıplardaki kanal bağımlılığı etiketi pakete taşınır ve silinmez (Bölüm 7.5); Faz 1'de bu etiketin çalışma zamanındaki tek karşılığı kullanım talimatının son satırıdır. Deterministik bir filtre K-05'in ikinci seçeneğidir ve evi Marka DNA tarafındaki `channels` kararıdır (Bölüm 3.2).
...
- **Fallback:** Faz 1 önlemi, her enjeksiyon bloğunun başına konan sabit kullanım talimatıdır (Bölüm 4.6). ⚠️ **Dürüst sınır — iki hakemde de yazılı:** bu bir **talimattır, deterministik filtre değildir**; tam çözüm marka kanal envanterine bağlıdır ve evi Marka DNA tarafındadır (**K-05**).
...
- **Çakışma/öncelik kuralı:** Acil olmak **onay kapısını ve sıra kuralını kaldırmaz**; taslak → aktif sırası ile yönetici son onayı korunur. *(Bir belge onay adımını bu akış için açıkça yazar; diğeri akışı aynı taslak → aktif mekanizmasına bağlar ve o mekanizmanın onay kapısı Bölüm 8'de ortak hükümdür.)* ⚠️ **Aşağı akışın motoru içermesi K-22'ye bağlıdır** — motor ilk fazda yoksa o zincir fiilen mevcut değildir ve akış motorsuz ön koşul zincirine düşer (Bölüm 8). Bu, K-22'nin sonucudur; **bu bölümde yeni bir karar açılmamıştır.**
...
| Marka kanal envanteri | **Marka DNA'sının `channels` alan adayı** | **K-05**'in tam çözümü; bu işte yalnız kullanım-talimatı düzeyi önlem | DNA tarafındaki karar belirler |
...
Bu alt başlık **iki ayrı doğrulama kümesi** taşır ve ikisi aynı statüde değildir: **(a)** yürürlükteki iş kurallarının pozitif/negatif/sınır senaryoları, **(b)** politika motorunun fixture testleri — ikincisinin tamamı **motorun benimsenmesine (K-22)** ve motorun kendi açık kararlarına bağlıdır.
...
**Politika motoruna bağlı olaylar** — motor benimsenirse (**K-22**), yalnız bir hakem belgesinde: motor sürümü ve sonuç durumu · içerik özetleri · karar kapsamı, değişim oranları ve blok nedenleri · yöneticiye gösterilen anlık görüntü ve son onay/ret olayı.
...
| Motor: kanıtsız çıkarma kararı | **Uygulanmıyor — kalıp korunuyor**; koşu raporunda *"kanıt yetersiz"* | Politika motoru testi | **Motor benimsenirse (K-22)** |
| Motor: çıkarılanlar listesiyle eşleşen kalıp | **Geri-ekleme çelişkisi** olarak işaretleniyor, sessizce eklenmiyor | Politika motoru testi | **Motor benimsenirse (K-22)** |
...
| 10 | Canonical diff, değişim oranları ve bariyer sonuçları üretildi | **K-22** + üç bariyerin ayrı ayrı benimsenmesi + canonical hash kuralı — hepsi açık (Bölüm 6.4, 7.7) |
...
- **Tetikleyici ve sıklık.** Kaynakta periyot **3 aydır**; iki hakem belgesi de **3 veya 6 ay** esnetmesini kaydeder ve ikisi de kesin değeri spec'e bırakır. **Periyodun global mi sektör bazlı mı yapılandırılacağı K-26'da açıktır.** Turu koşan **yöneticidir**; koşuyu hangi yüzeyden başlatacağı **K-27**'de açıktır.
...
⚠️ **Altıncı bir revizyon ihtiyacı karara bağlıdır, drift değildir:** politika motoru benimsenirse (**K-22**) sentez sözleşmesinin *"karar merci sensin"* rol hükmü revize edilmelidir — motor kararları doğrulayan bir katman olarak sentezin üstüne biner.
...
| Yönetici koşu yüzeyi (**K-27**) | Açık — rolün işi hangi yüzeyden yapacağı belirsiz |
...
- ⚠️ **Aynı envanter, koşu yüzeyini bir komut ailesinin adıyla eksikler arasında sayar.** Yüzeyin ne olacağı **K-27**'de açıktır — diğer hakem komut ailesini varsayar, bu hakem açık karar olarak kaydeder. **Aktarılan son duruma göre** eksik olan **koşu yüzeyidir** `[BU SENTEZDE DOĞRULANMADI]` — iddia bu sentezde ölçülmemiştir ve Bölüm 17'nin ⑤ kalemine bağlanmıştır; belirli bir komut ailesinin eksikliği bu belgede olgu olarak yazılmaz.
- ⚠️ **Zorunlu iş ile karara bağlı iş ayrılmalıdır.** **Aktarılan son duruma göre** kurulmamış bileşenlerin bir kısmı açık kararlara bağlıdır — alt sektör teyit bileşeninin **yerleşimi** **K-19**'da, üretim sürüm damgasının **veri yeri** **K-07**'de, koşu yüzeyinin **ne olacağı** **K-27**'de açıktır. ⚠️ **Politika motoru bu aktarılan kümenin üyesi DEĞİLDİR** — `[SEA-2026-08-11]` bileşenidir ve 2026-07-11 taraması onu **aramamıştır**; kaynakta bulunmadığı ölçülmüştür (provenans), **güncel varlığı bu sentezde ölçülmemiştir** `[BU SENTEZDE DOĞRULANMADI]`. Onun için açık olan **fazıdır** (**K-22**). **Buna karşılık iki bileşenin kurulması karara bağlı değildir, zorunludur** (Bölüm 5.1'de *Yeni*): içerik doğrulayıcısının kendisi zorunludur — **K-01b'ye bağlı olan yalnız özel gün anahtar kuralıdır**; prompt yakalama düzeneğinin kurulması da zorunludur — **K-20 düzeneğin varlığını değil, Marka DNA işiyle paylaşılmasını belirler**. Zorunlu iş açık kararın sonucuna çevrilmez.
...
| **K-27** | Yönetici turu hangi yüzeyden koşulacak? | A) Claude Code komut ailesi / B) Otomaix'te yönetici paneli | **[AÇIK]** — fark bir iş kalemidir: panel seçilirse ayrı bir arayüz geliştirmesi doğar. Komut ailesinin bugün var olup olmadığı doğrulanmadı; A'nın maliyeti buna bağlı | Ürün sahibi | Spec kapsamı sabitlenmeden önce | **Spec öncesi kullanıcı kararı** — yüzey seçimi işin kapsamını ve maliyetini belirler; işletim prosedürü (14) ve rol tasarımı (15) buna dayanır |
...
| **K-30** | Gerçek kullanım sinyali elde edilmeden ilerlenecek mi? | A) Sinyalsiz ilerlenir / B) Aktivasyon gerçek marka gelene kadar bekletilir | **[AÇIK]** — K-29'dan ayrı seçilebilir: kontrollü marka kabul edilip aktivasyon yine bekletilebilir. *"Faz 1 boyunca sinyal elde edilemez"* ölçülmemiş bir tahmindir | Ürün sahibi | `[TANIMSIZ]` | **Spec öncesi kullanıcı kararı** — risk kabulüdür; aktivasyonun Faz 1'de yapılıp yapılmayacağını belirler |
...
| **K-90** | `tur durduruldu` öğesi `bloklandı` ile birleştirilecek mi? | A) Birleştirilir / B) Ayrı kalır | **[AÇIK]** — iki kaynak katmanı sonucu farklı seviyelerde modeller (biri koşu seviyesi, diğeri kalıp-kararı seviyesi); tamamlayıcıdır, karşıt değildir — ama birleştirme kararı açıktır | Teknik sahip | `[TANIMSIZ]` | **Koşullu** — politika motoru Faz 1'e alınırsa (K-22) gerekli; motorsuz turda bu ayrım doğmaz |
| **K-91** | İlk paket koşusunda `değişiklik yok` sonucu geçersiz sayılacak mı? | A) Geçersiz sayılır / B) Sayılmaz | **[AÇIK]** — tek katmanda ele alınmıştır; diğerinde kavramın karşılığı yoktur — reddedilmiş değil, hiç ele alınmamıştır | Teknik sahip | `[TANIMSIZ]` | **Koşullu** — politika motoru Faz 1'e alınırsa (K-22) gerekli; motorsuz turda bu ayrım doğmaz |
...
| **K-95** | Politika sonucu ve koşu kaydı nerede tutulacak? | A) Artefakt tablosunda yeni tür / B) Paket tablosunda JSONB / C) Ayrı değerlendirme tablosu | **[AÇIK]** — raporun varlığı motorlu modelde ortaktır, yeri açıktır | Teknik sahip | `[TANIMSIZ]` | **Koşullu** — motor Faz 1'e alınırsa (K-22) veri yeri spec'te tanımlanır |
| **K-96** | Motor sentez kararını değiştirdiğinde özgün sentez, nihai aday ve gerekçeli fark ayrı mı saklanacak? | A) Ayrı saklama sözleşmesi tanımlanır / B) Sentez raporu ham katmana tek artefakt olarak yazılır, ayrım tutulmaz | **[AÇIK]** — iki konum birbirini dışlamaz; bu sentez birini seçmez | Teknik sahip | `[TANIMSIZ]` | **Koşullu** — motor Faz 1'e alınırsa (K-22) gerekli |
...
| **K-97** | Motorun sürümü ve yapılandırması her koşuya damgalanacak mı? | A) Damgalanır / B) Damgalanmaz | **[AÇIK]** — alternatif model aynı ihtiyacı karar satırı seviyesinde karşılar. K-25'e indirgenemez | Teknik sahip | `[TANIMSIZ]` | **Koşullu** — motor Faz 1'e alınırsa (K-22) gerekli |
...
| **K-05** | Kanal-bağımlı kalıplar için Faz 1'de hangi önlem alınacak? | A) Faz 1: kullanım talimatına tek satır / B) Faz 1'de kanal envanteri de kurulsun | **[AÇIK] — öneri:** **A'ya eğilim** — kanal envanterinin doğal evi Marka DNA işidir ve orada bir alan adayı olarak tanımlıdır; Faz 1'de tam çözüm kapsam dışıdır | Yönetici / operatör | `[TANIMSIZ]` | **Spec öncesi kullanıcı kararı** — Faz 1 kapsam kararıdır; (B) seçilirse Marka DNA işinden bir alan bu işe çekilir |
...
| **K-130** | Değişim büyüklüğü bariyeri benimsenecek mi? | A) Benimsenir / B) Benimsenmez | **[AÇIK]** — eşik değeri kararın parçası değil sonucudur; benimsenirse pilotta ölçülür (K-24) | Ürün sahibi | `[TANIMSIZ]` | **Koşullu** — motor Faz 1'e alınırsa (K-22) gerekli |
| **K-131** | Ekleme oranı bariyeri benimsenecek mi? | A) Benimsenir / B) Benimsenmez | **[AÇIK]** — K-130'dan ayrı seçilebilir | Ürün sahibi | `[TANIMSIZ]` | **Koşullu** — motor Faz 1'e alınırsa (K-22) gerekli |
| **K-132** | Kararsızlık oranı bariyeri benimsenecek mi? — motorun kararsız bıraktığı madde oranı eşiği aşarsa tur durur | A) Benimsenir / B) Benimsenmez | **[AÇIK]** — K-130 ve K-131'den ayrı seçilebilir | Ürün sahibi | `[TANIMSIZ]` | **Koşullu** — motor Faz 1'e alınırsa (K-22) gerekli |
| **K-133** | Kuru mod zorunlu olacak mı? — motorun kararlarını uygulamadan önce yalnız raporlaması | A) Zorunlu / B) Zorunlu değil | **[AÇIK]** — bariyerlerden ayrı seçilebilir | Ürün sahibi | `[TANIMSIZ]` | **Koşullu** — motor Faz 1'e alınırsa (K-22) gerekli |
| **K-134** | Motor işletime alınırken eski doğrudan operatör-onay akışı ne olacak? | A) Eski akış motor kanıtlanana kadar paralel korunur / B) Motor işletime alınırken eski akış kaldırılır *(kaynak katmanı seçenek saymaz; kararın kendi iki kutbu yazılmıştır)* | **[AÇIK]** — paralel koşum operasyon yükü doğurur, tek geçiş ise motorun kanıtlanmadan tek yol olmasını kabul eder | Ürün sahibi | `[TANIMSIZ]` | **Koşullu** — motor Faz 1'e alınırsa (K-22) gerekli; geçiş planının parçasıdır |
...
| **R-07** | Marka gerçeği ihlali — sahip olmadığı kanala ait çağrı kalıbı markaya basılır | **Y** | **Y** — sahte yönlendirme; uydurma yasağının ihlali | Kanal bağımlılığı etiketinin pakete taşınması + talimat satırı; tam çözüm Marka DNA kanal alanıdır (**K-05**, evi belirlenmiştir) | Kör örneklem değerlendirmesi — ⚠️ eşik açık (**K-11 (b)**) | Kalıbı kanal nötr uyarla; kanal alanı işini öne al | Operatör + teknik sahip ⚠️ | **Ortak + kaynakta karara bağlı** |
...
| **R-15** | Operasyon yükü ölçeklenmez — üç aylık tur × paket sayısı elle koşuluyor | **Y** | O — turlar aksar, paketler bayatlar | Faz 1 aktif paket tavanı (**≤5**) — ⚠️ **kaynakta öneridir**, ilk turda ölçülüp revize edilecektir (**K-13**) | Tur başına operasyon süresi metriği (Bölüm 13.6) | Tavanı düşür ya da otomasyonu öne al — ⚠️ motorun fazı **K-22**'de açıktır | Operatör | **Ortak + kaynakta karara bağlı**. ⚠️ *"İnsan eliyle aylar sürer"* gerekçesi `[ÖLÇÜLMEMİŞ VARSAYIM]`'dır, **kapıya çevrilmez ve ayrı karar açmaz** (Bölüm 2.1) |
...
| **R-22** | Motorun kararsızlık oranı yüksek çıkar — iş insana geri döner, otomasyon kazancı buharlaşır | O | O | Kararsızlık oranı metriği · motorun karar veremediği maddede güvenli varsayılan (**K-23** açık) · kalibrasyon. ⚠️ **Kararsızlık oranı bariyerinin benimsenmesi de açık karardır** (Bölüm 7.7) | Koşu raporu metriği | Kuralları genişlet ya da kapsamı daralt; gerekirse motoru sonraki faza al (**K-22**) | Teknik sahip ⚠️ | ⚠️ **Yalnız bir hakemde**, sonradan eklenen analiz |
| **R-23** | Ölçek gerekçesi doğrulanmamış — motorun varlık gerekçesi ölçülmemiş iki iddiaya dayanır | O | O — yanlış önceliklendirme | Pilotta **yöneticinin tur başına gerçek süresinin** ölçülmesi (Bölüm 13.6); hedef sektör sayısındaki belirsizliğin çözülmesi (**K-21**) | İlk turun süre kaydı | Ölçüm gerekçeyi desteklemiyorsa motorun kapsamı ve fazı yeniden değerlendirilir (**K-22**) | Ürün sahibi ⚠️ | ⚠️ **Yalnız bir hakemde**, sonradan eklenen analiz. **`[ÖLÇÜLMEMİŞ VARSAYIM]` — kapıya çevrilmez ve ayrı karar ID'si almaz** (Bölüm 2.1); besledigi kararlar **K-21** ve **K-22**'dir |
...
2. ⚠️ **Motorlu ↔ motorsuz ayrımı her senaryoda taşınır — K-22.** 19.1–19.5 **her iki modelde de** geçerlidir. **19.6 · 19.7 · 19.8 yalnız motorlu modelde geçerlidir**; motor ilk faza girmezse o üç senaryo **hiç oluşmaz** — kayıttan düşmezler, koşulamazlar.
...
- Markanın sahip olduğu bilinmeyen bir kanala bağlı çağrı kalıbı kullanılmaz — Faz 1'de bunun karşılığı **talimat satırıdır, deterministik filtre değildir**; tam çözüm **K-05**'e bağlıdır ve evi Marka DNA tarafındadır.
...
⚠️ **Bu senaryo ve onu izleyen ikisi hedef modeldir ve yalnız motorlu modelde geçerlidir.** Motorun hangi fazda geleceği **K-22**'de açıktır. Motor ilk faza girmezse aşağıdaki akış **oluşmaz** — o modelde aday paketi sentez üretir ve akış doğrudan biçim doğrulayıcısına ve yönetici onayına geçer (Bölüm 7.7).
...
**When:** Yönetici sektörü seçer, raporları teslim eder ve turu başlatır. ⚠️ **Turun hangi yüzeyden başlatılacağı K-27'de açıktır**; **bu satır yüzey seçmez.** Koşu klasörü ve teslim sözleşmesi **K-17** kapsamında öneri statüsündedir.
...
⚠️ **Bağımlılık:** bu kalemin ağırlığı **K-22**'ye bağlıdır. Politika motoru ilk faza girmezse `[SEA-2026-08-11]` katmanının provenansı **motor spec'i yazılana kadar** kritik olmaz; ilk faza girerse mimarinin en büyük eklentisi **kayıtsız bir artefakta** dayanıyor olur. **İki durumda da kalem düşmez, ağırlığı değişir.**
...
| 2026-08-21 | **K-30 KAPANDI — A: gerçek kullanım sinyali beklenmez; aktivasyon Faz 1'de yapılır** (risk kabulü) | Kullanıcı kararı; risk kabulüdür. **K-29'dan bağımsızdır** — K-29 (pilot markası) açık kalır. *"Faz 1 boyunca sinyal elde edilemez"* iddiası ölçülmemiş bir tahmindi; karar o tahmine değil, risk kabulüne dayanır. ⚠️ Gövdede K-30 atfı bulunmaz; Bölüm 17 satırı final sweep'te güncellenir | Kullanıcı (ürün sahibi) | **8.2 · 16.2** |
...
>
> ⚠️ **2026-08-21 kapsam turu:** **K-22 · K-27 · K-30 · K-05** kullanıcı kararıyla kapandı (**Ek B**); gövdedeki ve bu notun maddelerindeki *"açıktır"* ifadeleri bu dört karar için **bayattır** — Bölüm 17 satırları ve gövde final sweep'te güncellenir.
...
9. **Kullanım talimatı satırının blok şablonuna yazılması** — dağarcık kullanım kuralı (**K-04**, kapalı karar) ve marka gerçeği filtresi (**K-05**).
10. **Politika motoru** — kural seti, kanıt/eşik/bayrak kontrolleri, *"kararsız"* davranışı (**K-23**), eşiklerin kalibrasyonu (**K-24**), karar günlüğünün aktör alanı (**K-25**), motorun `active` satırına dokunamaması (**K-28**, kapalı karar). ⚠️ **Kuru modun benimsenip benimsenmeyeceği K-133'te açıktır.** ⚠️ **Motorun fazı K-22'de açıktır**; motor Faz 1'e girmezse bu madde politika motoru spec'ine kayar.
...
12. **Yönetici koşu yüzeyi** — komut ailesi mi yönetici paneli mi (**K-27**; kapsamı doğrudan belirler), yönetici ile müşteri arasındaki yetki ayrımı (**R-24**).
...
- Kapsam dışı maddelerin **evinin belli olması** — ölçülen durum: Bölüm 3.2'deki 10 maddenin **9'unda** yeniden ele alma koşulu veya adlandırılmış ev yazılıdır, **bakım borçları maddesinde yoktur** (Bölüm 20). **K-05**'in marka DNA dokümanına bağlanması korunur.
...
- **Yönetici koşu yüzeyinin nerede yaşayacağı belirsizdir** ve kapsamı doğrudan etkiler (**K-27**).
