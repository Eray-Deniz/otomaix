...
- **Post-review hardening pass** (2026-05-31, /review + /security-review sonrası): 6 bulgu sistemik (canonical blok 4-way + frozen spec mirror). Manuel inline edit yerine mekanik propagation (canonical 1 kez düzenlenip script'le 4 dosyaya kopya) + body-mirror için spec doc & command identical edit + her adımda Check A/B + body-diff yeniden doğrulama. **Frozen spec artık refine edildi** (R-kuralF relabel + Orta#1 slugify spec doc'a da işlendi → byte-freeze kalktı; yeni baseline). Canonical CODEX-CALL-PROTOCOL bloğu 45→66 satır (md5 `2503b639`). → Vault: [[decisions/2026-05-26-spec-writeplan-review-gated-hardening]] (decision-extend, 4-komut + Invariant #12 + güvenlik hardening + FIXED_FILES; vault commit `d4766a7`). **Closure'da vault promotion önce atlandı, kullanıcı uyarısıyla düzeltildi** — bkz. HANDOFF Verification DÜZELTME.
