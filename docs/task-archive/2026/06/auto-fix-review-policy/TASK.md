...
**KAPANDI (done, 2026-06-04).** `/finish-branch-claude-codex` mainline closure: 4 commit origin/main'e push, task arşivlendi, Auto-Fix Policy kararı vault'a promote edildi (`[[decisions/2026-06-04-auto-fix-review-policy]]`). Zincir: spec→plan→execute→review (5-tur fix)→security (SR-1 hardening)→closure, hepsi temiz. **Değişen deliverable:** komut dosyaları repo-DIŞı (pre-fix yedekler: review-cycle `*.bak-20260604T084344Z`, SR-1 hardening `*.bak-20260604T095430Z`). Repo commit bekleyen: review + security raporları + codex logları + TASK/HANDOFF (+ önceki Check D 58cb02e). push YOK.
...
- 2026-06-04: **Auto-Fix Review Policy → Vault promote edildi** (closure'da, kullanıcı onayı): `[[decisions/2026-06-04-auto-fix-review-policy]]` (vault commit 6444a2c, lokal). Politika davranışı + "binding≠prosedür" dersi kalıcı karar olarak vault'ta.
