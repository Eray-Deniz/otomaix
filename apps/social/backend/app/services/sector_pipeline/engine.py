...
def _iddiasiz_atiflar(atiflar: list[str], tasiyan: set[str]) -> list[str]:
    """Beyan edilen iddiaların HİÇBİRİNİ taşımayan atıflar.

    Ayrı bir fonksiyondur ki mutasyon kolu BİRİNCİ katmanı sökebilsin: sayım
    daraltması (İKİNCİ katman) ancak bu kapı devre dışıyken tek başına
    ölçülebilir. Aksi hâlde "savunma derinliği" ölçülmemiş bir iddia kalırdı.
    """
    ...
    * `cogunluk-yok` — referans çözüldü, satırın kaynak sayısı tabanın altında.
    ...
    for satir in _karar_satirlari(inputs):
        ...
        if olumsuz:
            ...
            continue
        # BAYRAK BLOĞU 2026-09-20'de KALDIRILDI — dayanağı ölçülerek yanlış
        # bulundu. Blok, denetçinin TİPLİ `bayraklar` sütununda tüketilmesi
        # gereken bir bayrak görünce açık soru üretiyordu. Ne var ki o sütuna
        # bayrağı DENETÇİ yazar ve bayrak orada durmaya devam eder; tüketim bir
        # sonraki adımda, SENTEZDE olur. Sütun bu yüzden "tüketilmedi"nin
        # kanıtı olamaz: blok bayraklı HER satırda ateşliyordu (canlı koşuda
        # ölçüldü: ürettiği bulguların HEPSİNDE pakete girecek metin temizdi) ve sentezin
        # sözleşmeye UYMASINI bloklayan kilidin ikinci yarısıydı.
        # Tüketimin ölçülebilir yüzeyi pakete girecek METİNDİR ve onu
        # `_bayrak_tuketimi` ölçer — kural TEK yerde yaşar.
        # Bilgi BLOKLAMADAN kayda geçer (2026-09-20, bağımsız review turu).
        # Blok kaldırıldığında `AuditRow.bayraklar` alanını okuyan üretim kodu
        # KALMAMIŞTI (ölçüldü) — yani "sentez bayrak kuralını görmezden geldi"
        ...
        if kayitli_bayraklar:
            bulgular.append(
                BulguIzi(
                    sinif="bayrak_kaydi",
                    unit_id=satir["unit_id"],
                    detay=(
                        f"andığı denetçi satırı tüketilmesi gereken bayrak "
                        f"taşıyordu: {kayitli_bayraklar} — bayrak başına kuralın "
                        f"gözetildiği MEKANİK olarak ölçülmez; kayıt operatörün "
                        f"denetimi içindir, BLOKLAMAZ"
                    ),
                )
            )
        celiskili = sorted(
        ...
        if celiskili:
            ...
            continue
        # SAYIM YALNIZ İDDİA TAŞIYAN SATIRLARDAN toplanır (F2). `cozulen`in
        # tamamını kullanmak, beyan edilen iddiayla hiçbir bağı olmayan bir
        # satırın kaynaklarını çoğunluğa katardı. Yukarıdaki çift yönlü kapı
...
def _tek_kaynak_istisnasi(
    ...
    * **(2) CANLI URL DOĞRULAMASI — en az bir denetçi O İDDİANIN URL'sini**
    ...
    # KAPSAMA ÖNCE, DEĞER SONRA (hakem turu 1, F5/yüksek — ÖLÇÜLDÜ). İlk yazım
    # yargıları iki raporun satırlarından TOPLUYOR ve "yazan herkes evet demiş
    # olmalı" diyordu; ama hiçbir katman profilin HER kaynağı kapsadığını
    # ölçmüyor (`_kaynak_profili` bunu R6 kapsam sınırı olarak açıkça beyan
    # eder). Ölçüldü: bir denetçi o kaynak için satırı HİÇ YAZMAZSA liste
    # `[True]` kalıyor, `all()` geçiyor ve istisna AÇILIYORDU — yani bir
    # denetçinin şüphesi, satırı yazmamasıyla susturulabiliyordu; docstring'in
    # kapattığını söylediği şeyin ta kendisi. Artık HER rapor o kaynak için
    # TAM BİR yargı taşımak zorundadır (eksik yargı = istisna KAPALI).
    yargilar: list[bool] = []
    for rapor in raporlar:
        rapor_yargisi = [
            profil.resmi
        ...
        if len(rapor_yargisi) != 1:
            return False
        yargilar.append(rapor_yargisi[0])
    if not all(yargilar):
        return False
    return any(
...
**Muafiyet FIAT DEĞİL, kanonik girdiden TÜRER (hakem turu 1, F7/orta).** İlk
yazım muafiyeti yalnız gerekçelendiriyor ama YETKİSİNİ göstermiyordu; iki hakem
de onu "uydurulmuş kural" saydı. Yetki spec girdisindedir ve AYNEN şöyle der:
*"`karma` kutlama ile ölçülü ticariyi birlikte taşır"*
(`docs/research/2026-08-21-sektor-bilgi-paketi-spec-input.md`, tür etiketinin
davranışsal karşılığı bölümü). Ticari ekseni HEM taşıyan HEM taşımayan bir
etiket, o eksende hiçbir değerle çelişemez — muafiyet tanımın kendisinden çıkar,
kontrolörün tercihinden değil.
"""
...
CHECKS: tuple[EngineCheck, ...] = (
    ...
    EngineCheck(
        ad="bayrak_tuketimi",
        aciklama=(
            "Pakete girecek metinde sağ çıkan tek bayrak `kanal-bagimli`; "
            "kalan yedi tüketilmiş olmalı."
        ),
...
ölü satırın aktif yolu, sıra kayması yüzünden adaydaki yaşayan bir yolla çakışıyor
ve bulgu ÇIKARILAN birime atfedilebiliyordu; (b) kanal bayrağının muafiyeti
çalışma zamanı filtresinin kapsamına DARALTILDI (Eray kararı) — CTA dışı yüzeydeki
kanal etiketi artık açık soru üretir; (c) BLOKLAMAYAN `bayrak_kaydi` sınıfı eklendi
...
YAZISINI değil, PAKETE GİRECEK öğe metnini tarar; (b) `yeni_oge_cogunlugu`'nun
denetçi `bayraklar` sütununu okuyan bayrak bloğu KALDIRILDI. Aynı girdi artık
farklı karar verir: bayrağını sözleşmeye uygun biçimde gerekçede tüketen bir
kalem eskiden açık soru üretip BLOKLARKEN artık sessizdir; pakete girecek
metninde bayrak kalan bir kalem eskiden sessizken artık açık soru üretir.
Ölçüldü (`kosu-222706dc…`): açık soruların tamamı bu iki kapıdan geliyordu ve
bayrak taşıyan satırların hiçbirinde pakete girecek metin kirli değildi —
ölçüm ve UZLAŞTIRMA kaydı: `docs/active/sektor-bilgi-paketi-plan2/K134-MOTOR-KARSILASTIRMA.md` → "Bayrak kapılarının iki ölçümü".

**KAPSAM SINIRI, dürüst etiket (2.17.0'ın aynısı):** parmak izi bu
değişikliği de YAKALAMAZ — sebep kümesi, kural kimlikleri, kontrol adları ve
not sınıfları DEĞİŞMEDİ; değişen şey bir kontrolün TARADIĞI YÜZEY ve bir
bloğun varlığı. Damga elle artırıldı.
...
ETKI_KAYIT = "yalniz-kayit"
"""Sonuca DOKUNMAYAN etki — bulgu raporlanır, sebep listesine GİRMEZ.

2026-09-20'de eklendi. `decide` döngüsünün `else` kolu o güne dek tanınmayan
her etkiyi aktivasyon engeline çeviriyordu; kayıt-etkisi bu yüzden AÇIK bir
kol ister — sessizce `else`'e düşse bloklamayan sınıf bloklardı.
"""
...
BULGU_ETKILERI: tuple[BulguEtkisi, ...] = (
    ...
    # YALNIZ KAYIT: sebep dizesi rapor/denetim içindir, sebep listesine GİRMEZ.
    BulguEtkisi("bayrak_kaydi", ETKI_KAYIT, "bayrak-kaydi"),
)
"""Bulgu → sonuç dönüşüm tablosu — `BULGU_SINIFLARI`'nın YEDİSİ de burada.
    ...
    for etki in BULGU_ETKILERI:
        ...
        if etki.etki == ETKI_BLOKLAR:
            ...
            if config.block_on_legislation:
                sebepler.append(etki.sebep)
        elif etki.etki == ETKI_KAYIT:
            # Sonuca DOKUNMAZ. Bu kol AÇIK yazılır: `else`'e düşseydi kayıt
            # sınıfı aktivasyonu engelleyen bir sınıf gibi davranırdı (2026-09-20).
            continue
        else:
